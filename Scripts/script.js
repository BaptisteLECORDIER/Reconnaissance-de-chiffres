...
async function predictModel(xs){
    ...
    for(let i = 0 ;i < 10; i++){
        document.getElementsByClassName("result")[i].innerHTML = ` ${i} `;
        document.getElementsByClassName("result")[i].innerHTML += `:<br> ${Math.round(Math.round(outputData[i] * 1000000) / 10000)}%`;
        if(highestScore[1] === i) {
            document.getElementsByClassName("result")[i].style.color = 'rgb(64, 33, 241)'
        }
        else {
